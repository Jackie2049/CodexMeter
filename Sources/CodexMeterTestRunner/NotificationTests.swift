...
private func testShortWindowFiresAtRemaining20Once() throws {
    ...
    try expectTrue(first[0].message.contains("5小时"), "message names the window")
...
private func testLongWindowFiresAtRemaining10() throws {
    ...
    try expectTrue(fired[0].message.contains("周度"), "message names weekly window")
...
private func testProShapeSingleWeeklyWindow() throws {
    ...
    try expectTrue(fired[0].message.contains("周度"), "label derived from duration")
}
